...
import {State, Style} from '@/core/types/design-request'
import {
  bookSizes,
  coverTypes,
  embellishmentLevels,
...
export function bookFactory(props?: BookFactoryProps) {
  return new Book({
    ...
    design_request: {
      ...
      style:
        props?.design_request?.style || styles[parseInt(faker.helpers.arrayElement(Object.keys(styles))) as Style].slug,
      ...
      image_filtering_level:
        props?.design_request?.image_filtering_level || faker.helpers.arrayElement(imageFilteringLevels),
      embellishment_level:
        props?.design_request?.embellishment_level || faker.helpers.arrayElement(embellishmentLevels),
